...
resource "aws_subnet" "public" {
  count                   = var.az_count
  cidr_block              = cidrsubnet(aws_vpc.dev-vpc.cidr_block, 8, var.az_count + count.index)
  availability_zone       = data.aws_availability_zones.available.names[count.index]
  vpc_id                  = aws_vpc.dev-vpc.id
...
resource "aws_nat_gateway" "dev-natgw" {
  ...
  allocation_id = element(aws_eip.dev-eip.*.id, count.index)
}
...
resource "aws_route_table" "private" {
  ...
  route {
    ...
    nat_gateway_id = element(aws_nat_gateway.dev-natgw.*.id, count.index)
  }
}
